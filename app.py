...
st.set_page_config(page_title="AstroPépites Pro v4.3", layout="wide")
...
with st.sidebar.expander("Obstacles", expanded=False):
...
db = [
    ...
    {"name": "M33 (Triangle)", "ra": "01:33:50", "dec": "+30:39:37", "type": "Galaxy", "size_w": 70, "size_h": 40, "cat": "Messier"},
    {"name": "NGC 2237 (Rosette)", "ra": "06:32:19", "dec": "+05:03:12", "type": "Emission", "size_w": 80, "size_h": 80, "cat": "NGC / IC"},
    ...
    {"name": "LDN 1235 (Shark)", "ra": "22:13:14", "dec": "+73:14:41", "type": "Dark", "size_w": 50, "size_h": 40, "cat": "Pépites Rares"},
]

# --- APP ---
st.title("🔭 AstroPépites Pro v4.3")
...
with t_radar:
    # --- DOUBLE MENU DÉROULANT ---
    col_cat, col_obj = st.columns(2)
    
    with col_cat:
        selected_cat = st.selectbox("📂 1. Choisir un catalogue", ["Tout", "Messier", "NGC / IC", "Pépites Rares", "Planètes"])
    
    # Filtrage de la base de données
    if selected_cat == "Planètes":
        filtered_objs = ["Jupiter", "Saturn", "Mars"]
    else:
        filtered_objs = [t['name'] for t in db if selected_cat == "Tout" or t['cat'] == selected_cat]
    
    with col_obj:
        active_target_name = st.selectbox("🎯 2. Choisir l'objet à analyser", ["--- Sélectionner ---"] + filtered_objs)

    if active_target_name != "--- Sélectionner ---":
        st.markdown("---")
        
        # Cas spécial Planètes
        if selected_cat == "Planètes" or active_target_name in ["Jupiter", "Saturn", "Mars"]:
            p_coord = get_body(active_target_name.lower(), now)
            ...
            with col1:
                st.image(f"https://alasky.u-strasbg.fr/hips-image-services/hips2fits?hips=CDS%2FP%2FDSS2%2Fcolor&ra={p_coord.ra.deg}&dec={p_coord.dec.deg}&width=450&height=450&fov=0.1&format=jpg", use_container_width=True)
            with col2:
                st.subheader(f"🪐 {active_target_name} {'✅' if visible else '❌'}")
                st.write(f"📍 Altitude : **{round(altaz.alt.deg)}°** | ✨ Filtre : **RGB / IR-Pass**")
            with col3:
                st.metric("Mode", "Vidéo / Lucky")
        ...
        # Cas DSO
            ...
            with col2:
                ...
                st.write(f"📍 Alt : **{round(altaz.alt.deg)}°** | ✨ Type : **{t['type']}**")
                
                if t['type'] == "Galaxy":
                    st.markdown('<div class="boost-box">🚀 **Expert Boost** : Ajoutez du H-Alpha pour les régions HII !</div>', unsafe_allow_html=True)
            ...
            with col3:
                ...
                st.write(f"📐 Champ : {round(fov_w)}' x {round(fov_h)}'")
    else:
        st.info("Choisissez un catalogue puis un objet pour lancer l'analyse.")

# Onglets Météo et Batterie identiques
with t_meteo:
    try:
